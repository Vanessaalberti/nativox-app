...
- `listarOperadores()` · `invitarOperadores(personas)` (devuelve los códigos, una sola vez) · `actualizarOperador(id, datos)` · `nuevoCodigoDeOperador(id)` · `borrarOperador(id)`
...
- **Lo usan:** `acceso`, `crear-evento`, `paneles`, `salas`, `agenda`, `operadores`.
